...
as $$
  with demand as (
    select
      se.normalized_query,
      (array_agg(se.query_text order by se.created_at desc))[1] as query_text,
      ...
      count(*) filter (
        where se.previous_search_event_id is not null
          and previous.normalized_query is not null
          and previous.normalized_query <> se.normalized_query
      )::bigint as follow_up_searches,
      max(se.created_at) as latest_search_at
    from public.search_events se
    left join public.search_events previous on previous.id = se.previous_search_event_id
    where se.normalized_query is not null
  ...
  select
    ...
    case
      ...
      when m.intent = 'calculation' and (m.clicked_searches::numeric / greatest(m.searches, 1)) < 0.35 then 'tool_candidate'
