...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const [{ data: questions }, { data: categories }, { data: tags }, { data: tools }] = await Promise.all([
  ...
  return [
    ...
    ...(tags ?? []).map((tag) => ({ url: `${siteUrl}/tags/${tag.slug}`, changeFrequency: 'weekly' as const, priority: 0.5 })),
