...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const [questionResult, toolResult] = await Promise.all([
    term && supabase ? supabase.from('questions').select('slug,title,body_markdown,view_count').in('status', ['open', 'closed']).or(`title.ilike.%${term}%,body_markdown.ilike.%${term}%`).order('view_count', { ascending: false }).limit(20) : Promise.resolve({ data: [] as { slug: string; title: string; body_markdown: string; view_count: number }[] }),
